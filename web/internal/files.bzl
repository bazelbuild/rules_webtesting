...
def _runfiles(ctx, deps_attrs=[], files=[]):
  ...
  The configured runfiles object will always have collect_data and
  collect_default set to True.
  ...
  for attr in deps_attrs:
    ...
    if hasattr(ctx.files, attr):
      transitive_files = transitive_files | set(getattr(ctx.files, attr))
  return ctx.runfiles(
      files=files,
...
def _get_transitive_files(attr_val):
  ...
  Args:
    attr_value: value of the attribute to collect runfiles from.
  Returns:
    set of Files.
  """
  transitive_files = set()
  ...
  if hasattr(attr_val, 'data_runfiles'):
    transitive_files = transitive_files | set(attr_val.data_runfiles.files)
  if hasattr(attr_val, 'default_runfiles'):
    transitive_files = transitive_files | set(attr_val.default_runfiles.files)
  return transitive_files
...
files = struct(runfiles=_runfiles, long_path=_long_path)
